...
SCHEDULER.every '5m', :first_in => 0 do |job|
  https = Net::HTTP.new("https://api.forecast.io", 443)
  https.use_ssl = true
  https.verify_mode = OpenSSL::SSL::VERIFY_PEER
  response = https.request(Net::HTTP::Get.new("/forecast/#{forecast_api_key}/#{forecast_location_lat},#{forecast_location_long}?units=#{forecast_units}"))
